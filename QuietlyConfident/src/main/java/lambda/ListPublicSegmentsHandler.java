...
public class ListPublicSegmentsHandler implements RequestHandler<Object,AllVideoSegmentsResponse> {
	 ...
	 * @throws Exception 
	 */
	List<VideoSegment> getPublicVideoSegments() throws Exception {
		logger.log("in getPublicVideoSegments");
		VideoSegmentsDAO dao = new VideoSegmentsDAO();
		
		return dao.getPublicVideoSegments();
	}
	...
	public AllVideoSegmentsResponse handleRequest(Object input, Context context)  {
		...
		try {			System.out.println("here");
			...
			List<VideoSegment> list = getPublicVideoSegments();
